...
if [[ -z "${GITLAB_CI}" ]]; then
  if [[ "$(git branch --show-current)" == "master" ]]; then
    echo ".gitlab/scripts/generate-component-with-graveyard-pipeline.sh 1"
    INSTANCES_DELETED=$(git diff --name-only --diff-filter D HEAD^ HEAD | grep ".*/${ENV}/.*/*.tfvars") || true
  else
    echo ".gitlab/scripts/generate-component-with-graveyard-pipeline.sh 2 branch $(git branch --show-current)"
    INSTANCES_DELETED=$(git diff --name-only --diff-filter D master..$(git branch --show-current) | grep ".*/${ENV}/.*/*.tfvars") || true
  fi
else
  if [[ "$CI_COMMIT_REF_SLUG" == "master" ]]; then
    echo ".gitlab/scripts/generate-component-with-graveyard-pipeline.sh 3"
    INSTANCES_DELETED=$(git diff --name-only --diff-filter D HEAD^ HEAD | grep ".*/${ENV}/.*/*.tfvars") || true
  else
    env
    echo ".gitlab/scripts/generate-component-with-graveyard-pipeline.sh 4"
    git fetch
    INSTANCES_DELETED=$(git diff --name-only --diff-filter D $CI_MERGE_REQUEST_DIFF_BASE_SHA HEAD | grep ".*/${ENV}/.*/*.tfvars") || true
  fi
fi
